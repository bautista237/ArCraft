 ...
 * and model fallback chain come from the {@code [gemini]} section of the TOML config; free-tier
 * keys vary a lot in which models they can use, so transient errors (429/503) retry with
 * backoff and then fall through to the next model in the chain.
 */
public final class GeminiService {

    public static final GeminiService INSTANCE = new GeminiService();
    ...
    public boolean isConfigured() {
        return ArcraftConfig.geminiConfigured();
    }

    private List<String> modelChain() {
        List<String> chain = new ArrayList<>();
        for (String m : ArcraftConfig.GEMINI_MODELS.get().split(",")) {
            String t = m.trim();
    ...
    public String ask(String question) {
        ...
        try {
            String context = AiContextService.INSTANCE.buildContext();
            String prompt = """
    ...
    private HttpResponse<String> call(String model, String payload) throws Exception {
        String url = "https://generativelanguage.googleapis.com/v1beta/models/"
                ...
                + URLEncoder.encode(ArcraftConfig.GEMINI_API_KEY.get(), StandardCharsets.UTF_8);
